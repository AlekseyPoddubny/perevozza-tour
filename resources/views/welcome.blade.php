            ...
            <div x-data="{ showTooltip: false }" class="relative bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden hover:border-gold/50 transition-all group">
                <div class="p-6 relative">
                    ...
                    <div class="flex justify-center items-center mb-6">
                        <div class="bg-gold/10 text-gold text-[20px] font-bold px-3 py-1 rounded-full uppercase tracking-tighter border border-gold/20">
                            {{ $schedule->departure_at->format('d.m.Y') }}
                            {{ $schedule->departure_at->format('H:i') }}
                        </div>
                    ...
                    <div class="flex justify-between items-start mb-6">
                        ...
                        <div class="text-right">
                            @if(!empty($schedule->price_rub))
                                <!-- РУБЛИ есть → они главные -->
                                <p class="text-xl font-bold text-white leading-none">
                                    {{ number_format($schedule->price_rub, 0, '.', ' ') }} ₽
                                </p>

                                @if(!empty($schedule->price_eur))
                                    <p class="text-[10px] text-gray-500 mt-1">
                                        {{ number_format($schedule->price_eur, 0) }} €
                                    </p>
                                @endif

                            @elseif(!empty($schedule->price_eur))
                                <!-- РУБЛЕЙ НЕТ → евро становятся главными -->
                                <p class="text-xl font-bold text-white leading-none">
                                    {{ number_format($schedule->price_eur, 0) }} €
                                </p>
                            @endif
                                ...
                                <div class="py-1">
                                    <div
                                        ...
                                        <button
                                            ...
                                            class="text-[10px] uppercase font-bold text-gold underline decoration-dotted"
                                        >
                                            Пункты сбора
                                        </button>
                    ...
                    <div class="grid grid-cols-2 gap-4 py-4 border-t border-zinc-800/50 mb-6">
                        <div>
                            <p class="text-[10px] text-gray-500 uppercase">Водитель</p>
                            <p class="text-[11px] font-semibold">
                                {{ $schedule->driver->full_name ?? 'Не назначен' }}
                            </p>
                        </div>
                        <div>
                    ...
                    <div x-show="showTooltip"
                         ...
                         class="absolute bottom-[76px] left-6 right-6 z-50"
                         style="display: none;">

                        <div class="bg-black/95 border border-gold/30 rounded-xl shadow-2xl backdrop-blur-md overflow-hidden">

                            <p class="text-gold/50 text-[8px] uppercase font-bold tracking-widest border-b border-white/5 py-2 text-center">
                                Пункты сбора
                            </p>
